...
function openVariantFromSection(section: SectionCatalogDoc): VariantSeed {
  return {
    baseKey: section.key,
    scope: normalizeContentScope(section.content_scope),
    sectionTitle: section.section_title || section.name || "",
    navTitle: String(section.in_page_nav_title || section.name || ""),
  };
}
...
function sectionPageKeys(section: SectionCatalogDoc): string[] {
  const keys: string[] = [];
...
type CategoryBadgeProps = {
  category: string;
  categories?: SectionCategoryOption[];
};

function CategoryBadge({ category, categories = [] }: CategoryBadgeProps) {
  const label =
...
export default function CmsSectionsPage() {
  ...
  const [sections, setSections] = useState<SectionCatalogDoc[]>([]);
  const [categories, setCategories] = useState<SectionCategoryOption[]>([]);
  const [pages, setPages] = useState<PageTemplateRecord[]>([]);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!alive) return;
        setSections((secRes.data || []) as SectionCatalogDoc[]);
        setPages((pageRes.data || []) as PageTemplateRecord[]);
        setCategories((catRes.data || []) as SectionCategoryOption[]);
      } catch (err) {
  ...
  const pageByKey = useMemo(() => {
    const map = new Map<string, PageTemplateRecord>();
  ...
  const categoryOptions = useMemo(
    () =>
      buildCategoryOptions(
        sections,
        categories.map((c) => ({ key: c.key, name: c.name || c.key }))
      ),
    [sections, categories]
  );
  ...
  function openVariantForm(section: SectionCatalogDoc | null = null) {
  ...
  async function onCreate(e: FormEvent) {
    ...
    const catalogEntry = SECTION_CATALOG.find((s) => s.key === pickKey);
    const meta = catalogEntry as ApiRecord | undefined;
    setSaving(true);
    setError(null);
    try {
      const res = await createSection({
        key: pickKey,
        name: String(meta?.name || catalogEntry?.name || pickKey),
        section_title: String(meta?.name || catalogEntry?.name || pickKey),
        in_page_nav_title: String(meta?.name || catalogEntry?.name || pickKey),
        category_key: String(meta?.category || catalogEntry?.category || ""),
        render_key: String(meta?.render_key || ""),
        content_scope: String(meta?.content_scope || "page"),
        status: true,
      });
      setPickKey("");
      setShowDevForm(false);
      router.push(
        `/cms/pages-content-sections/${(res.data as SectionCatalogDoc).key}`
      );
    } catch (err) {
      setError(err);
    } finally {
            ...
            {loading ? (
              ...
              <div className="space-y-6">
                {filteredSections.map((section) => {
                  const previewUrl = mediaUrl(String(section.section_preview_img || ""));
                  return (
                    <div
                      key={section.key}
                    ...
                    >
                      ...
                      {previewUrl ? (
                        <SectionPreviewThumb
                          src={section.section_preview_img as string | undefined}
                          alt={section.name}
                          natural
