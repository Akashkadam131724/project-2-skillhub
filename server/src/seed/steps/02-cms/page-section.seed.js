...
import { getSectionCategoryIdMap } from "../../../modules/cms/section-category.utils.js";
 ...
 * Usage:
 *   npm run seed:pages
 *   SEED_SAFE=1 npm run seed:pages   # upsert only — keeps sections & EntityPageSection
 */
...
function buildSectionDoc(s, pageByKey, catMap) {
  const pageTags = SECTION_PAGE_TAGS[s.key] || [];
  const catalog = getSectionCatalogMeta(s.key);
  const { category: _c, tags: _t, ...sectionFields } = s;
  return {
    ...sectionFields,
    section_category: catMap.get(catalog?.category || "") || null,
...
async function seedPagesAndSectionsSafe() {
  logSafeMode(
    "upsert page templates + add missing sections only (no wipe, no EPS changes)"
  );
  ...
  for (const s of SECTIONS) {
    const existing = await Section.findOne({ key: s.key });
    if (existing) {
      kept += 1;
      continue;
    }
    const doc = buildSectionDoc(s, pageByKey, catMap);
  ...
  console.log(
    `Safe pages seed: ${PAGES.length} templates · ${created} new sections · ${kept} existing unchanged`
  );
}

async function seedPagesAndSectionsReplace() {
  ...
  const sectionDocs = SECTIONS.map((s) =>
    buildSectionDoc(s, pageByKey, catMap)
  );
...
async function seed() {
  ...
  if (seedSafeMode()) {
    await seedPagesAndSectionsSafe();
  } else {
    await seedPagesAndSectionsReplace();
  }
...
const SECTIONS = [
  ...
  },
  {
    key: "feature_tabs",
    name: "Tabs — Vertical",
    description: "Vertical tabs with preview panel (supports nested items per tab)",
  ...
  },
  {
    ...
    description: "Horizontal pill tabs with content panel (nested items per tab)",
    section_title: "Explore by topic",
    in_page_nav_title: "Topics",
    sub_title: "Switch between paths without leaving the page.",
    button_title: "",
    target_url: "",
    data: {},
    content_scope: "page",
  ...
  },
  {
    key: "tabs_underline",
    name: "Tabs — Underline",
    description: "Editorial underline tabs with content panel (nested items per tab)",
    section_title: "Choose a path",
    in_page_nav_title: "Paths",
    sub_title: "Clean text tabs for longer narratives.",
    button_title: "",
    target_url: "",
    data: {},
    content_scope: "page",
  ...
  },
  {
    ...
    description:
      "Per-page customer quotes carousel — editable on each page (render_key: customer_testimonials)",
  ...
  },
  {
    ...
    section_title: "Special offer",
    sub_title: "",
    data: { open_delay_ms: 2500, storage_key: "promo_modal" },
    status: true,
  },
  {
    key: "newsletter_band",
    name: "Newsletter Band",
    description: "Email capture band with section buttons",
    section_title: "Subscribe",
    sub_title: "",
    data: { email_placeholder: "Work email" },
    status: true,
  },
  {
    key: "comparison_table",
    name: "Comparison Table",
    description: "Feature / format comparison with row CTAs",
    section_title: "Compare options",
    sub_title: "",
    status: true,
  },
  {
    key: "media_mosaic",
    name: "Media Mosaic",
    description: "Image mosaic grid with optional tile buttons",
  ...
  },
  {
    ...
    description: "Vertical journey timeline with step buttons",
  ...
  },
  {
    key: "trust_badges",
    name: "Trust Badges",
    description: "Compliance and trust logo grid",
    section_title: "Trust",
    sub_title: "",
    status: true,
  },
  {
    key: "split_cta",
    name: "Split CTA",
    description: "Image + copy split with section CTAs",
  ...
  },
  {
    key: "learning_path",
    name: "Learning Path",
    description: "Numbered learning path with per-step buttons",
  ...
  {
    key: "form_split",
    name: "Form — Split column",
    description: "Content column + static short lead form",
    section_title: "Get in touch",
    sub_title: "",
    data: { content_side: "left", form_key: "lead" },
    status: true,
  },
];
...
const SECTION_PAGE_TAGS = {
  ...
  feature_tabs: [
    { page_key: "home", sort_order: 13, status: true },
  ],
  key_benefits: [
