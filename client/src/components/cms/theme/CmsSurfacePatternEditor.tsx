...
function QuickBandChip({ preset, onClick }: QuickBandChipProps) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function BandRow({ band, index, total, onChange, onRemove, onMove }: BandRowProps) {
...
export default function CmsSurfacePatternEditor({
  ...
  function commit(next: SurfacePattern) {
    ...
    if (layout === "solid") {
      commit({
        ...
        bands: pattern.bands?.length
          ? [pattern.bands[0]]
          : defaultSurfacePattern().bands.slice(0, 1),
      });
      return;
    }
    commit({
  ...
  function updateBand(index: number, band: SurfacePatternBand) {
  ...
  function moveBand(from: number, to: number) {
  ...
  function addBand(preset?: QuickBandPreset) {
    const bands = [
      ...(pattern.bands || []),
      {
        id: newBandId(),
