...
/** Pure layout — rename TemplateBand → YourSection when copying. */
export default function TemplateBandUi({
  eyebrow = "Section",
  title,
  body,
  id,
  ...
  return (
    ...
    >
      <SectionWrapper>
        <div className="section-ui-card rounded-2xl border p-6 sm:p-8">
          {eyebrow ? (
          ...
          {title ? (
            <h2 className="section-theme-heading m-0 mt-3 text-2xl font-semibold tracking-tight sm:text-3xl">
              {title}
            </h2>
          ) : null}
          {body ? (
