...
function TeamMemberCard({ item }) {
  ...
  return (
    <article
      ...
      data-light-surface=""
      className="section-light-card flex h-full flex-col overflow-hidden rounded-2xl section-ui-card border"
    >
      ...
      <div className="flex flex-1 flex-col gap-2 p-4 sm:p-5">
        {name ? (
          <h3 className="m-0 text-lg font-semibold tracking-tight section-theme-heading">
            {name}
          </h3>
        ) : null}
        {role ? (
        ...
        {!isRichTextEmpty(bio) ? (
          <CmsRichText
            html={bio}
            className="mt-1 text-sm leading-relaxed section-theme-muted"
          />
        ) : null}
      </div>
    </article>
