...
import type { ItemFieldControlProps, ItemFieldDef } from "./types";
...
function FieldLabel({ field }: { field: ItemFieldDef }) {
  return (
...
function FieldHint({ field }: { field: ItemFieldDef }) {
...
export default function ItemFieldControl({
  ...
  const buttonValue = Array.isArray(raw) ? raw : [];
  const err = error || "";
    ...
    return (
      <div className="block text-sm">
        ...
        <div className="flex flex-wrap gap-1.5">
          {(field.options || []).map((opt) => {
            const active = String(stringValue || "") === String(opt.value);
            return (
              <button
      ...
      <div className="block text-sm">
        ...
        <div className="mb-3 grid grid-cols-2 gap-1.5 sm:grid-cols-4">
          {BANNER_GRADIENT_PRESETS.map((preset) => {
            const active =
              String(stringValue || "").replace(/\s+/g, "") ===
        ...
        <label className="flex flex-wrap items-center gap-2">
          ...
          {!isBannerGradient(stringValue) ? (
            <input
              type="color"
              value={
                /^#[0-9a-fA-F]{6}$/.test(String(stringValue || ""))
                  ? stringValue
                  : "#0b1f4d"
              }
              onChange={(e) => onChange({ [field.key]: e.target.value })}
  ...
  if (field.type === "buttons") {
    const btnCount = buttonValue.filter((b) => String(b?.label || "").trim()).length;
    return (
