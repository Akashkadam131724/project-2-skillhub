...
export function isCastProfileShowable(item: unknown): boolean {
...
export function toCastProfileUiItems(
...
export function resolveCastProfileUiItems(
  sectionKey: string,
  ...
  const list = options.cmsMode
    ? resolved
    : resolved.filter(isCastProfileShowable);
  return toCastProfileUiItems(list);
}
