...
export default function ThemeSettingsTab({ busy = false, onError }) {
  ...
  async function saveTemplateTheme() {
    ...
    try {
      ...
      onError?.(err.message || "Could not save template theme");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function clearTemplateTheme() {
    ...
    try {
      ...
      onError?.(err.message || "Could not clear template theme");
    } finally {
      setSaving(false);
    }
  }
