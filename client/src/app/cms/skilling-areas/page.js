...
import { createSkillingArea, deleteSkillingArea } from "@/lib/entity-cms-api";
import {
  CmsHeading,
  CmsPanel,
  StatusBadge,
...
export default function CmsSkillingAreasPage() {
  ...
  async function load(search = q) {
    setLoading(true);
    try {
      const res = await fetchSkillingAreas({
        limit: 100,
  ...
  return (
    <div>
      ...
      <CmsPanel title="All skilling areas">
        <div className="mb-4">
          <input
            className={inputClass}
            placeholder="Search skilling areas…"
            value={q}
        ...
        {loading ? (
          ...
          <EmptyState message="No skilling areas yet." />
        ) : (
