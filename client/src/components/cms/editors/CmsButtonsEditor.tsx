...
import type { ButtonDraft, CmsButtonsEditorProps } from "./types";
...
function emptyButton(sort_order = 0): ButtonDraft {
  return {
...
export function normalizeButtonsDraft(buttons: unknown): ButtonDraft[] {
...
export function serializeButtonsDraft(draft: ButtonDraft[] | null | undefined) {
  return (draft || [])
...
function reorder(list: ButtonDraft[], fromIndex: number, toIndex: number) {
  if (
    fromIndex === toIndex ||
    fromIndex < 0 ||
    toIndex < 0 ||
...
function ButtonDesignPreview({ button }: { button: ButtonDraft }) {
 ...
 */
export default function CmsButtonsEditor({ value = [], onChange }: CmsButtonsEditorProps) {
  const list = Array.isArray(value) ? value : [];
  const [dragIndex, setDragIndex] = useState<number | null>(null);
  const [overIndex, setOverIndex] = useState<number | null>(null);
  const dragIndexRef = useRef<number | null>(null);

  function commit(updater: (prev: ButtonDraft[]) => ButtonDraft[]) {
    onChange((prev) => {
  ...
  function updateAt(index: number, patch: Partial<CmsButtonData>) {
    commit((prev) =>
                ...
                >
                  {BUTTON_VARIANTS.map((v) => (
                    <option key={v} value={v}>
                      {BUTTON_VARIANT_LABELS[v as keyof typeof BUTTON_VARIANT_LABELS] || v}
                    </option>
                  ))}
                </select>
              </label>
                ...
                >
                  {BUTTON_ACTION_TYPES.map((t) => (
                    <option key={t} value={t}>
                      {BUTTON_ACTION_LABELS[t as keyof typeof BUTTON_ACTION_LABELS] || t}
                    </option>
                  ))}
                </select>
              </label>
            ...
            <div className="grid grid-cols-2 gap-2">
                ...
                >
                  {BUTTON_SIZES.map((s) => (
                    <option key={s} value={s}>
                      {BUTTON_SIZE_LABELS[s as keyof typeof BUTTON_SIZE_LABELS] || s}
                    </option>
                  ))}
                </select>
              </label>
              <label className="block text-sm">
                ...
                >
                  {BUTTON_SHAPES.map((s) => (
                    <option key={s} value={s}>
                      {BUTTON_SHAPE_LABELS[s as keyof typeof BUTTON_SHAPE_LABELS] || s}
                    </option>
                  ))}
                </select>
              </label>
                    ...
                    <optgroup key={group.id} label={group.label}>
                      {group.icons.map((iconKey) => (
                        <option key={iconKey} value={iconKey}>
                          {BUTTON_ICON_LABELS[iconKey as keyof typeof BUTTON_ICON_LABELS] || iconKey}
                        </option>
                      ))}
                    </optgroup>
                  ))}
                ...
                >
                  {BUTTON_ICON_POSITIONS.map((pos) => (
                    <option key={pos} value={pos}>
                      {BUTTON_ICON_POSITION_LABELS[pos as keyof typeof BUTTON_ICON_POSITION_LABELS] || pos}
                    </option>
                  ))}
                </select>
              </label>
