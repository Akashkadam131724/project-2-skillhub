...
type FiltersPanelProps = {
  ...
  toggleOption: (key: string, id: string, clearKeys?: string[]) => void;
  hasFilters: boolean;
  clearAll: () => void;
  isPending: boolean;
  className?: string;
...
/** Renders filter groups from the backend schema. Locked keys stay applied server-side. */
export default function CatalogFilters({
  ...
  const selectedByKey = useMemo(() => {
    ...
    for (const group of groups) {
      ...
      const lockedValue = lockedParams[group.key];
      const fromLocked =
        lockedKeySet.has(group.key) && lockedValue != null
          ? parseList(String(lockedValue)).map(normalizeId)
          : [];
  ...
  function toggleOption(key: string, id: string, clearKeys: string[] = []) {
