...
export function isBentoGridCellShowable(item: unknown): boolean {
...
export function toBentoGridUiItems(
...
export function resolveBentoGridUiItems(
  sectionKey: string,
  ...
  const list = options.cmsMode
    ? resolved
    : resolved.filter(isBentoGridCellShowable);
  return toBentoGridUiItems(list);
}
