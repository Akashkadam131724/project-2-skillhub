...
function ResultRow({ item, onSelect }: ResultRowProps) {
  const meta =
    TYPE_META[item.type as SearchResultType] || TYPE_META.content;
  const subtitle =
...
function EmptyState({ hasQuery, q }: EmptyStateProps) {
  if (!hasQuery) {
    return (
...
export default function HeaderSearch() {
  ...
  const [tab, setTab] = useState<SearchTabId>("all");
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const data = (await fetchGlobalSearch(
          { q, limit: 8 } as { q: string; limit?: number }
