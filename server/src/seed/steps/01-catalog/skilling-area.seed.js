...
async function seed() {
  ...
  const docs = SKILLING_AREAS.map((area) => ({
    ...area,
    status: "active",
  }));
  const { inserted, mode } = await replaceCatalogDocs(SkillingArea, docs, "slug");
  const areas = await SkillingArea.find({}).sort({ name: 1 }).lean();

  console.log(
    `${mode === "upsert" ? "Upserted" : "Seeded"} ${inserted} skilling areas (${areas.length} in DB):`
  );
