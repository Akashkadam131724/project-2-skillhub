...
export default function CmsSectionDetailPage() {
  const sectionKey = useKeyParam();

  const [section, setSection] = useState<SectionCatalogDoc | null>(null);
  const [meta, setMeta] = useState<SectionMetaForm>(emptyMeta);
  ...
  async function load() {
    ...
    try {
      ...
      const { section: data } = await ensureSectionRenderKeySaved(
        sectionRes.data as SectionCatalogDoc,
        updateSection
      );
      setSection(data);
      setMeta({
  ...
  return (
    <div>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start">
        <SectionPreviewThumb
          src={meta.section_preview_img || String(section.section_preview_img || "")}
          alt={section.name}
          className="h-28 w-44"
          rounded="rounded-xl"
        />
        <div className="min-w-0 flex-1">
          <CmsHeading
            title={String(section.name || sectionKey)}
