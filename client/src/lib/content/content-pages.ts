...
import { cmsEditHref } from "@/lib/cms/cms-edit-routes";
...
export type ContentDirectoryPath = keyof typeof CONTENT_DIRECTORY_BY_PATH;
 ...
 */
export function contentPathFromParams(slugParam: string | string[]) {
...
export function isReservedContentPath(slugParam: string | string[]) {
  const parts = Array.isArray(slugParam) ? slugParam : [slugParam];
...
export function contentPublicHref(
  contentOrPath: string | { path?: string } | null | undefined
) {
  let path = "";
...
export function contentCmsHref(contentOrPath: string | { path?: string }) {
