...
async function seed() {
  ...
  const { inserted, mode } = await replaceCatalogDocs(Product, docs, "slug");
  const total = await Product.countDocuments();
  console.log(
    mode === "upsert"
      ? `Upserted ${inserted} products · ${total} total in DB`
      : `Seeded ${total} products across ${vendors.length} vendors`
  );
