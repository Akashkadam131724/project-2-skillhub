...
async function seed() {
  ...
  console.log("Building course documents…");
  const docs = [];
  let courseIndex = 0;
  ...
  const { inserted, mode } = await replaceCatalogDocs(Course, docs, "slug");
  const total = await Course.countDocuments();
  console.log(
    `\n${mode === "upsert" ? `Upserted ${inserted.toLocaleString()} courses` : `Seeded ${inserted.toLocaleString()} courses`} · ${total.toLocaleString()} total in DB`
  );
