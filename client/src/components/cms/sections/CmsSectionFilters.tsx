...
import type { SectionCatalogEntry } from "@/components/cms/pages/types";

export function sectionCategory(section: SectionCatalogEntry | Record<string, unknown>) {
  const fromApi = String(
    section?.category_key || section?.category || ""
  ).toLowerCase();
  if (fromApi) return fromApi;
  const catalog = getSectionCatalogMeta(String(section?.key || ""));
  return String(catalog?.category || "").toLowerCase() || "uncategorized";
}

/** Legacy type buckets used by add-section pickers */
export function sectionKind(key?: string) {
...
export function FilterOption({ active, label, count, onClick }: FilterOptionProps) {
  return (
    <button
      type="button"
      onClick={onClick}
...
export function buildCategoryOptions(
  sections: Array<SectionCatalogEntry | Record<string, unknown>> = [],
  categories: Array<{ key: string; name: string }> | null = null
) {
  const catList =
    categories && categories.length
      ? categories.map((c) => ({ key: c.key, name: c.name }))
      : SECTION_CATEGORIES;
  const counts: Record<string, number> = { all: sections.length, uncategorized: 0 };
...
export function sectionScope(section: SectionCatalogEntry | Record<string, unknown>) {
