...
/**
 * @typedef {{ value: string, label: string }} ItemFieldOption
 * @typedef {{
 *   key: string,
 *   type: string,
 *   label: string,
...
function pickRuleOverrides(overrides = {}) {
  const out = {};
...
export function itemField(key, overrides = {}) {
 ...
 */
export function normalizeItemFields(fields, fieldLabels = {}, defaults = {}) {
...
export function getItemFieldDefs(config, { child = false } = {}) {
  if (!config) return [];
...
export function getItemFieldKeys(config, { child = false } = {}) {
  return getItemFieldDefs(config, { child }).map((f) => f.key);
}

export function findItemFieldDef(config, key, { child = false } = {}) {
...
function applyStringRules(schema, field, { measure } = {}) {
  const lengthOf =
...
function richtextSchema(field) {
  const requiredMsg =
    field.errors?.required || `${field.label} is required`;

  let schema = z.string();
  if (field.required) {
    schema = schema.refine((v) => !isRichTextEmpty(v), {
      message: requiredMsg,
    });
  } else {
    schema = schema.optional().or(z.literal(""));
...
export function buildItemFieldsZodSchema(fieldDefs = []) {
  const shape = {};
 ...
 */
export function validateItemFields(item, fieldDefs) {
...
export function validateSectionItem(item, config, { child = false } = {}) {
