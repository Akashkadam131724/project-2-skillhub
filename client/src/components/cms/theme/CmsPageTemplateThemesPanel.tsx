...
import type { CmsPageRow, CmsPageTemplateThemesPanelProps, PageThemeDraft } from "./types";

function hasTemplateOverrides(theme: PageThemeDraft | null | undefined) {
...
function surfaceLabel(theme: PageThemeDraft, siteTheme: PageThemeDraft) {
 ...
 */
export default function CmsPageTemplateThemesPanel({
  siteTheme,
}: CmsPageTemplateThemesPanelProps) {
  const [pages, setPages] = useState<CmsPageRow[]>([]);
  const [drafts, setDrafts] = useState<Record<string, PageThemeDraft>>({});
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const rows = (res.data || []) as CmsPageRow[];
      setPages(rows);
      const serverDrafts: Record<string, PageThemeDraft> = {};
      await Promise.all(
        rows.map(async (row) => {
          try {
  ...
  async function clearTemplate(key: string) {
