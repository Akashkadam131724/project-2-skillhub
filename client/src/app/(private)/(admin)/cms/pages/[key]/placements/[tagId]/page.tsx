...
export default function CmsPagePlacementDetailPage() {
  ...
  const [tag, setTag] = useState<PagePlacementTag | null>(null);
  const [sectionDoc, setSectionDoc] = useState<ApiRecord | null>(null);
  ...
  async function load() {
    ...
    try {
      ...
      const found = (tagsRes.data || []).find(
        (t: PagePlacementTag) => String(t.id) === placementId
      ) as PagePlacementTag | undefined;
      if (!found) {
      ...
      const sectionRes = await getSection(String(found.section_key));
      setPage(pageRes.data as PageTemplateRecord);
      setTag(found);
      setSectionDoc(sectionRes.data as ApiRecord);
  ...
  const scope = normalizeContentScope(
    tag?.content_scope || (sectionDoc?.content_scope as string | undefined)
  );
  ...
  async function saveContentPatch(patch: ApiRecord) {
    if (contentLocked) {
      throw new Error(lockedContentMessage(scope, "template"));
    }
    const body: ApiRecord = { ...patch };
  ...
  async function savePlacementMeta(e: FormEvent) {
    e.preventDefault();
    if (!tag) return;
    setSaving(true);
    setError(null);
  ...
  async function toggleStatus(visible: boolean) {
    setSaving(true);
    setError(null);
    try {
  ...
  return (
    <div>
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-start">
        <SectionPreviewThumb
          src={String(sectionDoc?.section_preview_img || "")}
