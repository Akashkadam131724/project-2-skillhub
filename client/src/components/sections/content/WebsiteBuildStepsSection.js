              ...
              {items.map((item, i) => (
                <li key={item._id || item.id || i} className="border-t border-slate-200 py-7 first:border-t-0 first:pt-0 dark:border-slate-800">
                  <div className="grid gap-4 sm:grid-cols-[5rem_1fr]">
                    ...
                    <div>
                      {item.title ? <h3 className="m-0 text-xl font-semibold tracking-tight section-theme-heading">{item.title}</h3> : null}
                      {item.subtitle ? <p className="mt-1 mb-0 text-sm font-medium text-brand">{item.subtitle}</p> : null}
                      {!isRichTextEmpty(item.body) ? <CmsRichText html={item.body} className="mt-3 text-[15px] leading-relaxed section-theme-muted" /> : null}
                    </div>
                  </div>
                </li>
              ))}
