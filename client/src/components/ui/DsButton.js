...
function ButtonContent({ button, showIcon = true }) {
  const normalized = normalizeButton(button);
  const action = resolveButtonAction(normalized);
  const iconKind = showIcon ? resolveButtonIcon(normalized, action) : null;
...
function resolveSurface(inverted, surface) {
  if (surface === "light" || surface === "dark") return surface;
  if (inverted === true) return "dark";
  return "inherit";
}

function commonAttrs(button, surfaceAttrs, customStyle) {
...
export default function DsButton({
  ...
  disabled = false,
  htmlType,
  /** { bg, fg, border, hoverBg, hoverFg, hoverBorder } — see buttonCustomStyle() */
  custom,
  style,
}) {
