...
export const BAND_SURFACE_PAGE_DEFAULT: BandSurfacePreset = {
  id: "page_default",
...
function solid(label: string, value: string, theme: "light" | "dark" = "light") {
  return {
...
function bandGrad(
  entry: { id: string; label: string; value: string; hint?: string },
  theme: "light" | "dark"
) {
  return {
    id: entry.id,
...
function mapGradList(
  list: Array<{ id: string; label: string; value: string; hint?: string }>,
  theme: "light" | "dark"
) {
...
export function normalizeBandBgValue(value: unknown) {
  return String(value || "")
    .trim()
    .replace(/\s+/g, " ")
    .toLowerCase();
...
export function matchBandSurfacePreset(bgColor: unknown) {
...
export function bandSurfaceSwatchStyle(bg: string) {
  if (!bg) {
    return {
      background:
...
export function applyBandSurfacePreset(preset: BandSurfacePreset | null | undefined) {
...
export function pageBandFillFromPreset(
  preset: BandSurfacePreset | null | undefined,
...
export function pageBandThemeForFill(bgColor: unknown) {
