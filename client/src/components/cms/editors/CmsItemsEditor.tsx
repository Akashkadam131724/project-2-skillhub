...
function emptyItem(sort_order = 0, extras: Partial<SectionItemDraft> = {}): SectionItemDraft {
  return {
    _key: nextDraftKey(),
    title: "",
    subtitle: "",
...
function itemStableId(item: SectionItemDraft) {
  return String(item?._id || item?.id || "").trim();
}

function isNestedChild(item: SectionItemDraft) {
  const type = String(item?.item_type || "").toLowerCase();
  if (type === "tab") return false;
  return Boolean(String(item?.parent_id || "").trim());
}

function isNestedTab(item: SectionItemDraft) {
  if (isNestedChild(item)) return false;
  const type = String(item?.item_type || "").toLowerCase();
  return type === "tab" || type === "";
}

export function normalizeItemsDraft(items: unknown): SectionItemDraft[] {
...
export function serializeItemsDraft(
  draft: SectionItemDraft[] | null | undefined,
  sectionKey: string,
  renderKey = ""
) {
  ...
  function isMongoId(value: unknown) {
    return /^[a-f\d]{24}$/i.test(String(value || ""));
  }

  function hasContent(item: SectionItemDraft, fields: string[]) {
...
export function validateItemsDraft(
  draft: SectionItemDraft[] | null | undefined,
  sectionKey: string,
  renderKey = ""
): ValidateItemsDraftResult {
  ...
  const errorsByKey: Record<string, Record<string, string>> = {};
  let ok = true;
...
function reorder(list: SectionItemDraft[], fromIndex: number, toIndex: number) {
  if (
    fromIndex === toIndex ||
    fromIndex < 0 ||
    toIndex < 0 ||
...
export default function CmsItemsEditor({
  ...
  const [openButtons, setOpenButtons] = useState<Record<string | number, boolean>>({});
  const [fieldErrors, setFieldErrors] = useState<Record<string, Record<string, string>>>({});
  /** Fields the user edited since last external validation — hide those errors */
  const [clearedFields, setClearedFields] = useState<Record<string, string[]>>({});
  ...
  function errorsForItem(itemKey: string | number) {
    const raw = {
      ...(errorsByKey?.[itemKey] || {}),
      ...(fieldErrors[itemKey] || {}),
    };
  ...
  function commit(updater: (prev: SectionItemDraft[]) => SectionItemDraft[]) {
    onChange((prev) => {
  ...
  function updateAt(index: number, patch: Partial<SectionItemDraft>) {
    commit((prev) =>
  ...
  function setItemButtons(
    index: number,
    buttonsOrUpdater: ButtonDraft[] | ((prev: ButtonDraft[]) => ButtonDraft[])
  ) {
    commit((prev) =>
      prev.map((item, i) => {
  ...
  function onDragStart(index: number, e: DragEvent<HTMLButtonElement>) {
  ...
  function onDragOver(index: number, e: DragEvent<HTMLDivElement>) {
    e.preventDefault();
    e.dataTransfer.dropEffect = "move";
    if (overIndex !== index) setOverIndex(index);
  }

  function onDrop(index: number, e: DragEvent<HTMLDivElement>) {
    e.preventDefault();
    const from =
  ...
  function roleBadge(item: SectionItemDraft) {
    if (!nested) return `Item`;
    if (isNestedChild(item)) return "Item";
    return "Tab";
  }

  function parentLabel(item: SectionItemDraft) {
    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        ...
        >
          {nested
            ? "+ Add tab"
            : `+ Add ${(config.label || "item").replace(/s$/i, "").toLowerCase()}`}
        </button>
