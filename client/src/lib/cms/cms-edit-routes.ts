...
const CMS_ENTITY_TYPES: Record<string, CmsEntityTypeConfig> = {
  vendor: {
    pageKey: "vendor",
    segment: "vendor",
...
const PUBLIC_EDIT_PATH_OVERRIDES: Record<string, string> = {
...
export function cmsPublicHref(pageKey: string, slugOrPath?: string) {
...
export function cmsEditHref(pageKey: string, slugOrPath?: string) {
...
export function cmsEditHrefFromPublicPath(pathname: string) {
...
export function exitCmsLiveEdit(router: CmsRouter, publicHref?: string | null) {
  if (publicHref) {
