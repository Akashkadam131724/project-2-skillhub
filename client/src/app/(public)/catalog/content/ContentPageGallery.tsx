...
export type ContentGalleryPage = {
  path: string;
  name?: string;
  description?: string;
  status?: string;
...
export type ContentPageGalleryProps = {
  pages: ContentGalleryPage[];
  title: string;
  subtitle?: string;
};
...
type GalleryModalProps = {
  ...
  index: number;
  onClose: () => void;
  onChangeIndex: (value: number | ((prev: number) => number)) => void;
};

function GalleryModal({ pages, index, onClose, onChangeIndex }: GalleryModalProps) {
  const page = pages[index];
  const total = pages.length;

  const goPrev = useCallback(() => {
    onChangeIndex((current) => (current - 1 + total) % total);
  }, [onChangeIndex, total]);

  const goNext = useCallback(() => {
    onChangeIndex((current) => (current + 1) % total);
  }, [onChangeIndex, total]);
...
function ContentCard({
  ...
  page: ContentGalleryPage;
  onOpenAt: (page: ContentGalleryPage) => void;
}) {
  const accent = accentForPath(page.path);
...
export default function ContentPageGallery({
  pages,
  title,
  subtitle,
}: ContentPageGalleryProps) {
  ...
  const flatFiltered = useMemo(() => {
    const out: ContentGalleryPage[] = [];
    for (const [, items] of groups) out.push(...items);
    return out;
  }, [groups]);

  function openAt(page: ContentGalleryPage) {
        ...
        {groups.map(([label, items]) => (
          <section key={label} className="mb-10">
            ...
            <ul className="m-0 grid list-none gap-4 p-0 sm:grid-cols-2 lg:grid-cols-3">
              {items.map((page: ContentGalleryPage) => (
                <li key={page.path}>
