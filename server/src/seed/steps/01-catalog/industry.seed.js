...
async function seed() {
  ...
  const docs = INDUSTRIES.map((item) => ({
    ...item,
    status: "active",
  }));
  const { inserted, mode } = await replaceCatalogDocs(Industry, docs, "slug");
  const industries = await Industry.find({}).sort({ name: 1 }).lean();

  console.log(
    `${mode === "upsert" ? "Upserted" : "Seeded"} ${inserted} industries (${industries.length} in DB):`
  );
