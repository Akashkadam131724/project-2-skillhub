...
export function stripHtmlClasses(html?: string | null) {
  return String(html || "")
 ...
 */
export function stripHtmlStyles(
  html?: string | null,
  { keepColors = false }: { keepColors?: boolean } = {}
) {
...
function unwrapEmptySpans(html: string) {
  let out = String(html || "");
  let prev = "";
  while (out !== prev) {
    prev = out;
...
export function stripHtmlColors(html?: string | null) {
  return stripHtmlStyles(html, { keepColors: false });
}

/** Strip tags for short previews / empty checks. */
export function richTextPlainPreview(html?: string | null, max = 160) {
