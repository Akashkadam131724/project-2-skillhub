...
function ChildCard({ item, onFormOpen, tone = "dark" }) {
  ...
  return (
    <article
      ...
      className={`flex h-full flex-col overflow-hidden rounded-2xl border ${dark
    ...
    >
      ...
      <div className="flex flex-1 flex-col p-4">
        {item?.title ? (
          <h4
            className={`m-0 text-base font-semibold tracking-tight ${dark ? "text-white" : "section-theme-heading"
              }`}
          >
            {item.title}
          </h4>
        ...
        {item?.subtitle ? (
          <p
            className={`mt-1 mb-0 text-sm ${dark ? "text-white/60" : "text-slate-500"
              }`}
          >
            {item.subtitle}
          </p>
        ...
        {!isRichTextEmpty(item?.body) ? (
          <CmsRichText
            html={item.body}
            className={`mt-2 text-sm ${dark ? "text-white/70" : "section-theme-muted"
              }`}
          />
        ) : null}
        {buttons.length ? (
...
function Panel({ current, childItems = [], onFormOpen, layout }) {
  ...
  return (
    <div
      className={`overflow-hidden rounded-[1.75rem] border shadow-lg ${lightPanel
          ? "border-slate-200 bg-white"
    ...
    >
      ...
      {lightPanel ? (
        photo ? (
          ...
          <div className="px-6 py-6 sm:px-8">
            {current?.title ? (
              <h3 className="m-0 font-[family-name:var(--font-display)] text-2xl font-semibold section-theme-heading sm:text-3xl">
                {current.title}
              </h3>
            ) : null}
            {!isRichTextEmpty(current?.body) ? (
              <CmsRichText
                html={current.body}
                className="mt-3 max-w-2xl text-sm section-theme-muted"
              />
            ) : null}
            {panelButtons.length ? (
              <SectionButtons
      ...
      {hasChildren ? (
        <div
          className={`border-t px-4 py-5 sm:px-6 sm:py-6 ${lightPanel
