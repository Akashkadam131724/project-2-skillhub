...
async function seed() {
  ...
  const { inserted, mode } = await replaceCatalogDocs(Vendor, docs, "slug");
  console.log(
    mode === "upsert"
      ? `Upserted ${inserted} vendors (safe — existing vendors kept)`
      : `Seeded ${inserted} SkillHub vendors`
  );
  console.log(
    `Products planned: ${docs.reduce((n, v) => n + (v.productCount || 0), 0)}`
  );
  console.log(
    `Courses planned: ${docs.reduce((n, v) => n + (v.courseCount || 0), 0)}`
  );
