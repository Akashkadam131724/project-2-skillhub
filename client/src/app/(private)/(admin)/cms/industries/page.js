...
export default function CmsIndustriesPage() {
  const {
    ...
    onSearchChange,
    reload,
  } = useCmsEntityList(fetchIndustries);
  ...
  async function onToggleStatus(item) {
    try {
      await updateIndustry(item.slug, {
  ...
  async function onRestore(item) {
    try {
      await restoreIndustry(item.slug);
      await reload();
    } catch (err) {
      setError(err);
    }
        ...
        renderItem={(item) => (
          <li key={item.slug} className={cmsEntityListRowClass()}>
            ...
            <CmsEntityRowActions
              ...
              publicHref={cmsPublicHref("industry", item.slug)}
              liveEditHref={cmsEditHref("industry", item.slug)}
