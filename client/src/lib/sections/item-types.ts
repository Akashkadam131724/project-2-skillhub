/** Keep in sync with server/src/modules/cms/item.schema.js */

import {
  resolveSectionBehaviorKey,
  sectionUsesItems,
} from "@/lib/sections/section-items-config";
import type { PlacementLike } from "@/lib/sections/section-types";
import { isRichTextEmpty } from "@/lib/utils/rich-text";

/** CMS section item row — loose shape shared across map helpers. */
export type ItemLike = Record<string, unknown>;

function itemId(item: ItemLike | null | undefined) {
  return String(item?._id || item?.id || "").trim();
}

function itemHasVisibleContent(item: ItemLike | null | undefined) {
  if (!item) return false;
  return Boolean(
    item.title ||
    item.subtitle ||
    !isRichTextEmpty(String(item.body ?? "")) ||
    item.label ||
    item.value ||
    item.image_url ||
    item.bg_color ||
    item.icon ||
    item.href ||
    (Array.isArray(item.buttons) && item.buttons.length) ||
    // Tabs can exist as shells that only group children
    String(item.item_type || "").toLowerCase() === "tab"
  );
}

/** True when this row is a nested child of a tab (or other parent). */
export function isChildItem(item: ItemLike | null | undefined) {
  if (!item || item.status === false) return false;
  const type = String(item.item_type || "").toLowerCase();
  if (type === "tab") return false;
  return Boolean(String(item.parent_id || "").trim());
}

/** True when this row is a top-level tab / legacy item. */
export function isTabItem(item: ItemLike | null | undefined) {
  if (!item || item.status === false) return false;
  if (isChildItem(item)) return false;
  const type = String(item.item_type || "").toLowerCase();
  // Explicit tab, or legacy top-level row (no item_type / no parent)
  return type === "tab" || type === "";
}

/** Active items sorted by sort_order (must have some visible content) */
export function sortActiveItems(items?: unknown): ItemLike[] {
  if (!Array.isArray(items)) return [];
  return [...items]
    .filter((item): item is ItemLike => {
      if (!item || typeof item !== "object" || (item as ItemLike).status === false) {
        return false;
      }
      return itemHasVisibleContent(item as ItemLike);
    })
    .sort(
      (a, b) =>
        Number((a as ItemLike).sort_order ?? 0) - Number((b as ItemLike).sort_order ?? 0)
    );
}

/**
 * Mapping `items[]` only — no legacy data.items / data.modules fallback.
 * Items live on EntityPageSection, never page tags or Section.data.
 */
export function resolveSectionItems(items?: unknown): ItemLike[] {
  return sortActiveItems(Array.isArray(items) ? items : []);
}

/**
 * Group flat items into tabs with children.
 * Legacy rows (no item_type / parent_id) become tabs with empty children.
 */
export function groupItemsByTabs(items?: unknown) {
  const all = resolveSectionItems(items);
  const tabs = all.filter((item) => isTabItem(item));
  const children = all.filter((item) => isChildItem(item));

  return tabs.map((tab) => {
    const id = itemId(tab);
    return {
      ...tab,
      children: children
        .filter((child) => String(child.parent_id || "").trim() === id)
        .sort(
          (a, b) =>
            Number(a.sort_order ?? 0) - Number(b.sort_order ?? 0)
        ),
    };
  });
}

/** Section keys that use flat tab → child nesting via item_type + parent_id */
export const NESTED_TABS_SECTION_KEYS = new Set([
  "feature_tabs",
  "tabs_vertical",
  "tabs_horizontal",
  "tabs_underline",
  "sarder_ecosystem",
]);

/** Resolve items for a placement — nested children hidden unless section wants them */
export function resolveItemsForSection(sectionKey: string, items?: unknown) {
  const active = resolveSectionItems(items);
  const key = String(sectionKey || "").toLowerCase();

  // Nested tab sections: return tab shells only (children via groupItemsByTabs)
  if (NESTED_TABS_SECTION_KEYS.has(key)) {
    return active.filter((item) => isTabItem(item));
  }

  // Other sections: ignore nested children so they don't leak into grids
  return active.filter((item) => !isChildItem(item));
}

/**
 * Item-driven sections are meaningless without items.
 * Returns false when the public page (cms !== true) should hide the whole section.
 */
export function placementHasRequiredItems(section: PlacementLike | null | undefined) {
  if (!section) return true;
  if (
    !sectionUsesItems(
      String(section.section_key || ""),
      section.render_key != null ? String(section.render_key) : undefined
    )
  ) {
    return true;
  }
  return resolveItemsForSection(String(section.section_key || ""), section.items).length > 0;
}

function hasActiveButtons(section: PlacementLike | null | undefined) {
  const buttons = section?.buttons as Array<Record<string, unknown>> | undefined;
  if (Array.isArray(buttons)) {
    const live = buttons.some(
      (b) =>
        b &&
        b.status !== false &&
        Boolean(
          String(b.title || b.label || "").trim() ||
          String(b.href || b.target_url || b.url || "").trim()
        )
    );
    if (live) return true;
  }
  return Boolean(
    String(section?.button_title || "").trim() ||
    String(section?.target_url || "").trim()
  );
}

/**
 * Field-driven sections (overview, most heroes…): need at least one display field.
 *
 * Public pages (`cmsMode` false): only **content** counts — title, subtitle, body,
 * section image, or CTAs. Band bg color / bg image alone must not show an empty shell.
 *
 * CMS mode: also treat band chrome as “present” so editors can open empty shells
 * (though `shouldRenderPlacement` already always returns true in CMS).
 */
export function placementHasFieldContent(
  section: PlacementLike | null | undefined,
  cmsMode = false
) {
  if (!section) return false;

  const hasCopyOrMedia = Boolean(
    String(section.section_title || "").trim() ||
      String(section.sub_title || "").trim() ||
      !isRichTextEmpty((section?.data as { body?: string } | undefined)?.body) ||
      String(section.section_img_url || "").trim() ||
      hasActiveButtons(section)
  );
  if (hasCopyOrMedia) return true;

  if (!cmsMode) return false;

  return Boolean(
    String(section.section_bg_img || "").trim() ||
      String(section.section_bg_color || "").trim() ||
      String((section?.data as { bg_color?: string } | undefined)?.bg_color || "").trim()
  );
}

/**
 * Context-backed sections always have runtime UI (catalog grid, products fetch…).
 * In-page nav builds from other visible sections — keep it if enabled.
 */
const CONTEXT_BACKED_SECTION_KEYS = new Set([
  "in_page_nav",
  "catalog",
  "entity_directory",
  "latest_blogs",
  "blog_directory",
  "products",
  "related_courses",
]);

/** Renders baked-in static UI — empty CMS fields still show on public pages. */
const STATIC_RENDER_SECTION_KEYS = new Set(["hero_gradient_slider"]);

/**
 * True when this placement has something real to show on a public page.
 * - Item sections → active items
 * - Field sections → title / body / media / buttons (not band color alone)
 * - Context sections → always (when status is on)
 */
export function placementHasMeaningfulContent(
  section: PlacementLike | null | undefined,
  cmsMode = false
) {
  if (!section) return false;
  const key = String(section.section_key || "").toLowerCase();
  const behavior = resolveSectionBehaviorKey(
    key,
    section.render_key != null ? String(section.render_key) : undefined
  );

  if (CONTEXT_BACKED_SECTION_KEYS.has(key)) return true;
  if (STATIC_RENDER_SECTION_KEYS.has(behavior)) return true;

  if (sectionUsesItems(key, section.render_key != null ? String(section.render_key) : undefined)) {
    const hasItems =
      resolveItemsForSection(key, section.items).length > 0;
    // Hero stats can be useful with copy alone (items optional in the UI)
    if (key === "hero_stats") {
      return hasItems || placementHasFieldContent(section, cmsMode);
    }
    // Vendor grid: left copy/buttons or CMS items; public falls back to built-in links
    if (behavior === "vendor_link_grid") {
      if (placementHasFieldContent(section, cmsMode)) return true;
      if (hasItems) return true;
      return !cmsMode;
    }
    return hasItems;
  }

  return placementHasFieldContent(section, cmsMode);
}

/**
 * Live pages: hide placements that have nothing to show.
 * CMS mode always shows shells so editors can fill empty sections.
 * Drives both section display and in-page nav (nav reads the filtered list).
 */
export function shouldRenderPlacement(
  section: PlacementLike | null | undefined,
  cmsMode = false
) {
  if (!section) return false;
  if (section.status === false) return false;
  if (cmsMode) return true;
  return placementHasMeaningfulContent(section, cmsMode);
}

/**
 * Build a probe object from section component props (for per-UI empty checks).
 */
export function sectionProbeFromProps(
  sectionKey: string,
  props: Record<string, unknown> = {}
) {
  return {
    section_key: sectionKey,
    section_title: props.section_title,
    sub_title: props.sub_title,
    data: props.data,
    section_img_url: props.section_img_url,
    section_bg_img: props.section_bg_img,
    section_bg_color: props.section_bg_color,
    buttons: props.buttons,
    button_title: props.button_title,
    target_url: props.target_url,
    items: props.items,
  };
}

/** FAQ: title/body (or legacy q/a) */
export function itemQuestion(item: unknown): string {
  const row = item as ItemLike | null | undefined;
  return String(row?.title || row?.q || row?.question || "");
}

export function itemAnswer(item: unknown): string {
  const row = item as ItemLike | null | undefined;
  return String(row?.body || row?.a || row?.answer || "");
}

/** Testimonial helpers */
export function itemQuote(item: unknown): string {
  const row = item as ItemLike | null | undefined;
  return String(row?.body || row?.quote || row?.text || "");
}

export function itemAuthor(item: unknown): string {
  const row = item as ItemLike | null | undefined;
  return String(row?.title || row?.label || row?.author || "");
}

/** Stat helpers */
export function itemStatValue(item: unknown): string {
  const row = item as ItemLike | null | undefined;
  return String(row?.value || "");
}

export function itemStatLabel(item: unknown): string {
  const row = item as ItemLike | null | undefined;
  return String(row?.label || row?.title || "");
}

/** Benefit / card title */
export function itemTitle(item: unknown): string {
  if (typeof item === "string") return item;
  const row = item as ItemLike | null | undefined;
  return String(row?.title || row?.label || "");
}
