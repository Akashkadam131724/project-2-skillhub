...
import type { BandDraft, CmsSectionBandEditorProps } from "./types";
...
function bandPreviewStyle(
  draft: BandDraft,
  themeInfo: { defaultBg?: string }
): CSSProperties | null {
...
export default function CmsSectionBandEditor({
  ...
  const set = (patch: Partial<BandDraft>) => onChange({ ...draft, ...patch });
    ...
    <form onSubmit={onSubmit} className="space-y-4">
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="min-w-0 flex-1">
          <CmsOverrideGuide variant={"band" as const} />
        </div>
        <CmsOverrideGuide variant={"drawer-button" as const} />
      </div>
