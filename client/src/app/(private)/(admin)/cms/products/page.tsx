...
import type { ApiRecord, ProductCreateForm, ProductListItem, SelectOption } from "../entity-types";
...
function vendorLabel(vendor: ProductListItem["vendor"]): string {
  if (!vendor || typeof vendor !== "object") return "—";
  return String((vendor as ApiRecord).name || "—");
}
...
export default function CmsProductsPage() {
  const {
    ...
    onSearchChange,
    reload,
  } = useCmsEntityList<ProductListItem>(fetchProducts);
  ...
  useEffect(() => {
    fetchVendors({ limit: 100, status: "active" })
      .then((res) => setVendors((res.data || []) as SelectOption[]))
      .catch(() => {});
  }, []);
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      ...
      setShowForm(false);
      await reload();
      window.location.href = `/cms/product/${(res.data as ProductListItem).slug}`;
    } catch (err) {
      setError(err);
    } finally {
      setSaving(false);
  ...
  async function onToggleStatus(item: CmsEntityListItem) {
    ...
    try {
      await updateProduct(String(product.slug), {
  ...
  async function onDelete(item: CmsEntityListItem) {
    ...
    try {
      await deleteProduct(String(product.slug));
      await reload();
    } catch (err) {
      setError(err);
    }
  ...
  async function onRestore(item: CmsEntityListItem) {
    const product = item as ProductListItem;
    try {
      await restoreProduct(String(product.slug));
      await reload();
    } catch (err) {
      setError(err);
    }
          ...
          <li key={product.slug} className={cmsEntityListRowClass()}>
            <div className={cmsEntityListPrimaryClass()}>
              ...
              <p className={cmsEntityListMetaClass()}>
                {vendorLabel(product.vendor)} · /product/{product.slug}
              </p>
            </div>
            <CmsEntityRowActions
              item={product}
              ...
              liveEditHref={cmsEditHref("product", String(product.slug))}
