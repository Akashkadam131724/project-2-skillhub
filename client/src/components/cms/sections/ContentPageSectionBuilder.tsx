...
import { CONTENT_PAGE_STARTERS } from "@/lib/content/content-page-starters";
...
function sectionScope(section: SectionCatalogEntry | Record<string, unknown>) {
  const raw = String(section?.content_scope || "").toLowerCase().trim();
  if (raw === "global" || raw === "template" || raw === "page") return raw;
  return "page";
}

function newRow(sectionKey: string): ContentPageSectionRow {
  return {
...
function PageSectionList({
  ...
  catalogByKey: Map<string, SectionCatalogEntry>;
  disabled: boolean;
...
export default function ContentPageSectionBuilder({
  ...
  function applyStarter(starterKey: string) {
