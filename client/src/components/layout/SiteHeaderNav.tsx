 ...
 */
function MegaPanel({ item, columns, onNavigate }: MegaPanelProps) {
        ...
        <div className="min-h-0 flex-1 overflow-y-auto overscroll-contain p-4 sm:p-5">
          {links.length ? (
            <ul className="m-0 grid list-none gap-3 p-0 sm:grid-cols-2">
              {links.map((link: NavLinkItem, i: number) => (
...
function MobileColumnList({ column, onNavigate }: MobileColumnListProps) {
...
function DesktopNav({ navigation }: DesktopNavProps) {
  ...
  useEffect(() => {
    function onDocClick(event: MouseEvent) {
      ...
      if (
        (t instanceof Node && navRef.current?.contains(t)) ||
...
type MobileNavSectionsProps = {
  navigation: NavigationItem[];
  onNavigate: () => void;
};

function MobileNavSections({ navigation, onNavigate }: MobileNavSectionsProps) {
  const [openId, setOpenId] = useState<string | null>(null);
