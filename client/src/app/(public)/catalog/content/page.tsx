...
import ContentPageGallery, { type ContentGalleryPage } from "./ContentPageGallery";
...
export default async function ContentPagesCatalogPage() {
  let pages: ContentGalleryPage[] = [];

  try {
    const all = await fetchAllContents();
    pages = (all as ContentGalleryPage[]).filter(isGalleryContentPage);
