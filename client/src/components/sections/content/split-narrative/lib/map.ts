...
import type { SplitNarrativeChapterUiItem } from "./types";
...
export function isSplitNarrativeChapterShowable(item: unknown): boolean {
...
export function toSplitNarrativeChapterUiItems(
  mappingItems: MappingItem[]
): SplitNarrativeChapterUiItem[] {
...
export function resolveSplitNarrativeChapterUiItems(
  sectionKey: string,
  ...
  const list = options.cmsMode
    ? resolved
    : resolved.filter(isSplitNarrativeChapterShowable);
  return toSplitNarrativeChapterUiItems(list);
}
