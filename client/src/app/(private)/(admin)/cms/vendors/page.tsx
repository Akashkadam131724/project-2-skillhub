...
import type { VendorCreateForm, VendorListItem } from "../entity-types";

const emptyForm: VendorCreateForm = {
  name: "",
  email: "",
  phone: "",
  status: "active",
...
export default function CmsVendorsPage() {
  const {
    ...
    onSearchChange,
    reload,
  } = useCmsEntityList<VendorListItem>(fetchVendors);
  ...
  async function onCreate(e: FormEvent) {
    ...
    try {
      ...
      setShowForm(false);
      await reload();
      window.location.href = `/cms/vendor/${(res.data as VendorListItem).slug}`;
    } catch (err) {
      setError(err);
    } finally {
      setSaving(false);
  ...
  async function onToggleStatus(vendor: VendorListItem) {
    try {
      await updateVendor(String(vendor.slug), {
  ...
  async function onDelete(vendor: VendorListItem) {
    if (!confirm(`Delete vendor "${vendor.name}"?`)) return;
    try {
      await deleteVendor(String(vendor.slug));
      await reload();
    } catch (err) {
      setError(err);
    }
  ...
  async function onRestore(vendor: VendorListItem) {
    try {
      await restoreVendor(String(vendor.slug));
      await reload();
    } catch (err) {
      setError(err);
    }
        ...
        renderItem={(vendor) => (
          <li key={vendor.slug} className={cmsEntityListRowClass()}>
            ...
            <CmsEntityRowActions
              ...
              liveEditHref={cmsEditHref("vendor", String(vendor.slug))}
              onToggleStatus={(item) => onToggleStatus(item as VendorListItem)}
              onDelete={(item) => onDelete(item as VendorListItem)}
              onRestore={(item) => onRestore(item as VendorListItem)}
            />
          </li>
        )}
      />
