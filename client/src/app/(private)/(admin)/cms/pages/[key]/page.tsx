...
type PlacementFormState = Omit<PlacementForm, "sort_order"> & {
  sort_order: number | string;
};

type PlacementEditForm = Omit<PlacementFormState, "section_key">;
...
export default function CmsPageDetailPage() {
  const pageKey = useKeyParam();
  const router = useRouter();

  const [page, setPage] = useState<PageTemplateRecord | null>(null);
  const [form, setForm] = useState<PageDetailForm | null>(null);
  const [themeForm, setThemeForm] = useState<PageThemeDraft>(emptyPageTheme());
  const [siteTheme, setSiteTheme] = useState<PageThemeDraft | null>(null);
  const [placements, setPlacements] = useState<PagePlacementTag[]>([]);
  const [allSections, setAllSections] = useState<SectionCatalogDoc[]>([]);
  const [addForm, setAddForm] = useState<PlacementFormState>(emptyPlacement);
  const [editingId, setEditingId] = useState<string | number | null>(null);
  const [editForm, setEditForm] = useState<PlacementEditForm | null>(null);
  ...
  const [pageTab, setPageTab] = useState<PageTab>("mapped");
  ...
  async function load() {
    ...
    try {
      ...
      setPage(pageRes.data as PageTemplateRecord);
      setForm({
        name: String(pageRes.data.name || ""),
        description: String(pageRes.data.description || ""),
        entity_type: String(pageRes.data.entity_type || ""),
        status: pageRes.data.status !== false,
        is_sort_disabled: pageRes.data.is_sort_disabled !== false,
      });
      setThemeForm(normalizePageTheme((pageRes.data as ApiRecord).theme));
      setSiteTheme((siteRes?.data as PageThemeDraft) || null);
      const tags = ((tagsRes.data || []) as PagePlacementTag[])
        .slice()
        .sort(
          (a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0)
        );
      setPlacements(tags);
      setAllSections((sectionsRes.data || []) as SectionCatalogDoc[]);
      setAddForm((f) => ({
        ...f,
        sort_order: tags.length
          ? Math.max(...tags.map((t) => t.sort_order ?? 0)) + 1
          : 1,
      }));
    } catch (err) {
      setError(err);
  ...
  async function addPlacement(e: FormEvent<HTMLFormElement>) {
    e.preventDefault();
  ...
  function startEdit(tag: PagePlacementTag) {
  ...
  async function saveEdit(e: FormEvent<HTMLFormElement>) {
    ...
    setError(null);
    try {
      await updatePageSectionTag(String(editingId), {
  ...
  async function toggleTag(tag: PagePlacementTag) {
    try {
      await setPageSectionTagStatus(String(tag.id), !tag.status);
      await load();
    } catch (err) {
      setError(err);
    }
  ...
  async function removeTag(tag: PagePlacementTag) {
    if (!confirm(`Remove ${tag.section_key} placement from ${pageKey}?`)) return;
    try {
      await deletePageSectionTag(String(tag.id));
      await load();
    } catch (err) {
      setError(err);
    }
                ...
                <form onSubmit={savePage} className="grid gap-3 sm:grid-cols-2">
                  ...
                  <Field label="Entity type">
                    <select
                      ...
                      onChange={(e) =>
                        setForm((f) =>
                          f ? { ...f, entity_type: e.target.value } : f
                        )
                      }
                    >
                      {ENTITY_TYPES.map((t) => (
                        ...
                        <div className="flex min-w-0 items-start gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="mt-1 mb-0 text-sm text-slate-600 dark:text-slate-300">
                              {String(tag.section_title || tag.section_name || "— inherits section defaults")}
                            </p>
                          </div>
                        </div>
                        ...
                        >
                          <Field label="Section title override">
                            <input
                              ...
                              onChange={(e) =>
                                setEditForm((f) =>
                                  f ? { ...f, section_title: e.target.value } : f
                                )
                              }
                          ...
                          <Field label="Subtitle override">
                            <input
                              ...
                              onChange={(e) =>
                                setEditForm((f) =>
                                  f ? { ...f, sub_title: e.target.value } : f
                                )
                              }
                            />
                          </Field>
                                  ...
                                  try {
                                    ...
                                    setEditForm((f) =>
                                      f
                                        ? {
                                            ...f,
                                            section_img_url: String(res.data?.url || ""),
                                          }
                                        : f
                                    );
                                  } catch (err) {
                    ...
                    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4">
                      {filteredAddOptions.map((s) => {
                        ...
                        return (
                          ...
                          >
                            <SectionPreviewThumb
                              src={String((s as ApiRecord).section_preview_img || "")}
                              alt={String(s.name || "")}
          ...
          <CmsPanel title="Page preview">
            <CmsPagePreviewStack
              ...
              items={placements.map((tag) => ({
                id: String(tag.id),
                section_key: String(tag.section_key || ""),
                sort_order: tag.sort_order,
                hidden: tag.status === false,
                content_scope: tag.content_scope as string | undefined,
                preview: String(
                  allSections.find((s) => s.key === tag.section_key)
                    ?.section_preview_img || ""
                ),
